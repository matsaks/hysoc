...
def plot_trajectory_comparison(out_path: str, raw_points: List[Point], oracle_items: List[object], hysoc_items: List[object], title: str):
    ...
    ax_raw, ax_oracle, ax_hysoc = axes.ravel()
    ...
    ax_raw.set_title(f"Raw Trajectory ({len(raw_points)} points)", fontsize=14)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Demo 17: HYSOC-G vs Offline Oracle Final Evaluation.")
    ...
    # Aggregate CSV
    csv_path = os.path.join(out_dir, "hysoc_g_evaluation_metrics.csv")
    if results:
    ...
    labels = ['Offline Oracle', 'HYSOC-G']

    axes[0].boxplot([oracle_cr, hysoc_cr], tick_labels=labels, showmeans=True)
    axes[0].set_title("Compression Ratio (CR)")
    axes[0].set_ylabel("Compression Ratio (Raw/Compressed)")
    axes[0].grid(True, alpha=0.3)

    axes[1].boxplot([oracle_sed, hysoc_sed], tick_labels=labels, showmeans=True)
    axes[1].set_title("Information Loss (Mean SED)")
    axes[1].set_ylabel("Synchronized Euclidean Distance (meters)")
    axes[1].grid(True, alpha=0.3)

    axes[2].boxplot([oracle_lat, hysoc_lat], tick_labels=labels, showmeans=True)
    ...
    plt.tight_layout()
    plot_path = os.path.join(out_dir, "hysoc_g_vs_oracle_comparison.png")
