"""HYSOC-G SQUISH+DP trade-off figure: CR vs mean SED, NYC | SF.

One panel per city showing the compression-versus-fidelity frontier at buffer
capacity beta = 200 across the DP-tolerance grid, with the 15 m mean-SED budget
marked.
"""
...
SWEEP_OUTPUT_ROOT = os.path.join(project_root, "results", "sweeps", "module3_hysoc_g_move")
...
def build_points(rows: list[dict], winner_eps: float) -> list[dict]:
    points = []
    for r in rows:
        if r["strategy"] != "squish_dp" or int(to_float(r["capacity"])) != OPERATING_BETA:
            continue
