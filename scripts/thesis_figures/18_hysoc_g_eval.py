"""HYSOC-G end-to-end evaluation figure: 2x4 per-trajectory distributions.

Boxplots comparing Plain DP, Baseline-G, and HYSOC-G across CR, mean SED,
Stop F1, and latency (rows: NYC, SF). The Stop F1 panel draws HYSOC-G only,
since Plain DP has no stops and Baseline-G is trivially 1.0.
"""
...
EVAL_OUTPUT_ROOT = os.path.join(
    project_root, "results", "experiments", "hysoc_g_eval"
)
OUTPUT_ROOT = os.path.join(project_root, "results", "figures")

NYC_RUN_DIR: str | None = "0515_1059"
SF_RUN_DIR: str | None = "0525_1604_op-sf_centre"

USE_LATEX = True
DPI = 300
FIGSIZE = (6.5, 8.0)

PIPELINES = ["plain_dp", "baseline_g", "hysoc_g"]
PIPELINE_LABELS = {
    "plain_dp": "Plain DP",
...
COLUMNS = [
    ...
    ("mean_sed_m", "Mean SED (m)", False, False),
...
PANEL_CAPS = {
    ...
    "mean_sed_m": 100,
    "latency_median_us_per_point": 10_000,
}
...
def _draw_stop_f1_panel(ax, hysoc_values: list[float]) -> None:
    if hysoc_values:
        ...
        bp["boxes"][0].set_facecolor(PIPELINE_COLORS["hysoc_g"])
        bp["boxes"][0].set_alpha(0.75)
    ax.axhline(1.0, linestyle="--", color="#888888", linewidth=0.9, alpha=0.8)
    ax.set_xticks([1])
    ax.set_xticklabels([PIPELINE_LABELS["hysoc_g"]])
    ax.set_xlim(0.3, 1.7)
...
def main() -> None:
    ...
    for city_label, city_slug in CITIES:
        ...
        n_traj_per_city[city_label] = sum(
            1 for r in traj_rows if r.get("pipeline") == "hysoc_g"
        )
    ...
    for row_idx, (key, title, log_scale, is_stop_f1) in enumerate(COLUMNS):
        for col_idx, (city_label, city_slug) in enumerate(CITIES):
            ...
            if is_stop_f1:
                _draw_stop_f1_panel(ax, data["hysoc_g"])
            else:
                _draw_box_panel(ax, data, log_scale=log_scale)
            if key in PANEL_CAPS:
                ax.set_ylim(top=PANEL_CAPS[key])
            if row_idx == 0:
    ...
    fig.suptitle(
        f"HYSOC-G evaluation -- per-trajectory distributions ({n_str})"
    )
