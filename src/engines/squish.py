"""SQUISH capacity-bounded SED-priority geometric compressor."""
...
class SquishCompressor:
    ...
    def compress(self, points: List[Point], capacity: Optional[int] = None) -> List[Point]:
        """Compress a point list with SQUISH, keeping at most `capacity` points."""
        if not points:
            return []
        ...
        for i in range(len(points)):
            ...
            if len(current_buffer_nodes) < target_capacity:
                ...
                # Buffer full: link new node to the end, then evict min-priority node
                last_node = current_buffer_nodes[-1]
                last_node.next = new_node
                new_node.prev = last_node
                ...
                # Lazy removal: skip stale heap entries
                while True:
