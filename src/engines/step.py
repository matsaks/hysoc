"""STEP streaming stay-point segmenter (Module I)."""
...
from constants.step_defaults import STEP_DEFAULT_GRID_FACTOR

def local_distance(p1: Point, p2: Point) -> float:
    """Fast flat-earth distance approximation in metres, adequate for short extents."""
...
class STEPSegmenter:
    """Streaming stay-point segmenter (STEP) that emits STOP and MOVE segments on the fly."""
    ...
    def __init__(
        ...
        # Not-yet-flushed points as (Point, gx, gy) tuples.
        self.cache: List[Tuple[Point, int, int]] = []
        self.cache_offset = 0  # Absolute index matching cache[0].

        # Absolute indices of the currently identified stay-point.
    ...
    def process_point(self, p_c: Point) -> List[Segment]:
        """Process a newly arrived point and emit any finished sub-trajectories."""
        segments = []
        ...
        while i >= self.cache_offset:
            ...
            if (delta_x + 1) ** 2 + (delta_y + 1) ** 2 <= self.threshold_sq:
                i -= 1  # confirmed in-range
            elif max(0, delta_x - 1) ** 2 + max(0, delta_y - 1) ** 2 > self.threshold_sq:
                i += 1  # pruned out of range, restore i to first valid
                break
            elif local_distance(p_c, p_i) <= self.max_eps:
                i -= 1  # exact check satisfies constraint
            else:
                i += 1  # exact check violates
                break
        ...
        if Is is not None:
            if self.current_sp_start is not None:
                if Is <= self.current_sp_end:
                    # Intersects the open stay-point: merge them.
                    self.current_sp_end = Ie
                else:
                    # Separated: flush the stop, then the in-between move.
                ...
                # First stay-point: flush anything before it as a move.
            ...
            if self.current_sp_start is not None:
                ...
                if local_distance(p_c, p_Ie) > self.max_eps:
                    # Moved away: flush the stop but keep the ongoing move cached.
                    sp1_points = self._get_points(self.current_sp_start, self.current_sp_end)
                    segments.append(self._create_stop(sp1_points))
    ...
    def flush(self) -> List[Segment]:
        """Emit remaining cached segments when the stream terminates."""
        segments = []
        if self.current_sp_start is not None:
    ...
    def process(self, trajectory: List[Point]) -> List[Segment]:
        """Batch-processing helper for testing and benchmarking."""
        segments = []
        for p in trajectory:
